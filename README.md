...
| `NEXT_PUBLIC_STAGE_BLUEPRINT_URL` | no, but wanted | `lib/stage-blueprint.ts` | Full URL of the backend's `stage_blueprint` endpoint. The plan set uploads *and starts its estimate* as soon as it is picked, so the final step has nothing left to send and the pipeline is already running. Unset ⇒ signup still works, the upload just happens at submit like before. |
