...
| `NEXT_PUBLIC_TURNSTILE_SITE_KEY` | no locally, **yes** deployed | `lib/turnstile.ts` | Cloudflare Turnstile site key. Staging now costs a pipeline run per call, so the backend verifies a token before starting one; the widget is invisible, like the upload it guards. Unset ⇒ no token is sent, which a dev backend (`IS_DEV_ENV`) waves through and a deployed one refuses with `CHALLENGE_FAILED`, falling back to uploading at submit. |
